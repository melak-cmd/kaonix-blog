---
title: "netshoot: a Docker + Kubernetes Network Troubleshooting Swiss-Army Container"
description: "Debug container networking without installing anything: netshoot gives you tcpdump, iperf3, mtr, nmap, tshark and 100+ tools inside any Docker or Kubernetes network namespace."
pubDate: 2026-08-24
category: "tutorials"
tags: ["docker", "kubernetes", "networking", "troubleshooting", "tools"]
author: "Kaonix Team"
---

[netshoot](https://github.com/nicolaka/netshoot) is a container packed with 100+ network troubleshooting tools. Its real power comes from where you can *run* it: inside any Docker or Kubernetes **network namespace**, so you can inspect traffic exactly where the problem lives — without installing a single package into your images or your hosts.
...
Your app image stays minimal and secure; the debugging toolbox is rented only when needed.
...
Attach netshoot as a capture sidecar next to your service:
...
Or add it as a sidecar in a Deployment, then exec into it:
...
⭐ NXDOMAIN means the record doesn't exist (check your Service names); a timeout points at kube-dns/CoreDNS itself.
...
⭐ Remember that `nc -vz` succeeding proves the TCP handshake works — it says nothing about the application layer on top.
...
⭐ `ip route get <ip>` is criminally underused — it answers "which interface and gateway would this packet actually use?" in one line.
...
### 1. Prefer ephemeral containers over sidecars in Kubernetes

`kubectl debug` adds netshoot to a running pod without restarting it and removes itself when done — zero impact on your workload spec.

### 2. Never bake debugging tools into production images

Every extra binary widens the CVE surface. Keep images minimal; rent netshoot when you need it.

### 3. Pin the image version in CI

`nicolaka/netshoot:latest` changes underneath you — pin a tag for reproducible diagnostics.

### 4. Combine with `kubectl get events` and `ss`

netshoot shows you the wire; the earlier workflow (`events → describe → ss`) tells you which wire to look at.

### 5. Watch the security context

`--net host` and `--privileged` are powerful — use them deliberately and never on multi-tenant nodes you don't control.
